...
export default function EventsPage() {
	...
	return (
		<main className="min-h-screen bg-white">
			{/* --- 1. HERO HEADER (Kao sa tvoje slike) --- */}
					...
					<div className="flex flex-col gap-6">
						...
						<div className="flex flex-col md:flex-row gap-4 items-center">
							...
							<Button
								variant="ghost"
								onClick={() => {
									setCategories([]); setCities([]); setSearch("");
						...
						{/* Red sa specifičnim filterima */}
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
							<div className="space-y-1.5">
								<Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Kategorija</Label>
								<EventCategoriesMultiSelect handleSelectChange={setCategories} selectedCategories={filters.categories} />
							</div>
							<div className="space-y-1.5">
								<Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Lokacija</Label>
								<CitiesMultiSelect handleSelectChange={setCities} selectedCities={filters.cities} />
							</div>
							<div className="space-y-1.5">
								<Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Od datuma</Label>
								<DateTimePicker value={fromDate} maxDate={toDate} onChange={(d) => setFrom(d?.toISOString() || null)} placeholder="Izaberite datum" />
							</div>
							<div className="space-y-1.5">
								<Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Do datuma</Label>
								<DateTimePicker value={toDate} minDate={fromDate} onChange={(d) => setTo(d?.toISOString() || null)} placeholder="Izaberite datum" />
							</div>
						</div>
					</div>
				</Card>
				...
				{!loading && events.length > 0 && (
					<div className="mt-24 flex justify-center">
						<div className="bg-slate-50 p-2 rounded-2xl border border-slate-100 shadow-sm">
							<PaginationComponent currentPage={urlPage} totalPages={totalPages} onPageChange={updatePage} />
						</div>
					</div>
				)}
			</section>
